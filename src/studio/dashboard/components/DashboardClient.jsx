...
export default function DashboardClient({ initialPosts = [] }) {
  ...
  return (
    <div className="p-10 max-w-[1240px] mx-auto min-h-screen bg-bg">
      <div className="flex items-center justify-between mb-10">
        <h1 className="text-[34px] font-serif text-ink tracking-tight">Posts</h1>
        <div className="flex items-center gap-4">
          <div className="relative">
            <input 
              type="text" 
              placeholder="Search posts" 
              className="bg-surface border border-border rounded-full py-2.5 px-5 text-sm w-64 focus:outline-none focus:border-ink transition-colors placeholder:text-muted-soft"
            />
          </div>
          <button 
            onClick={() => setShowNewPostDialog(true)}
            className="bg-ink text-surface px-6 py-2.5 rounded-full text-[14px] font-semibold hover:bg-ink/90 transition-colors flex items-center gap-2"
          >
