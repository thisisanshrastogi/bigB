      ...
      {steps.map((step, i) => {
          ...
          return (
            <div key={i} className="flex gap-6 relative">
              ...
              <div className="flex flex-col items-center shrink-0">
                <div className="w-[18px] h-[18px] rounded-full bg-accent mt-[6px] shrink-0 border-[4px] border-surface z-10" />
                {!isLast && <div className="w-[2px] bg-border flex-1 -mt-1 -mb-2" />}
              </div>
              
              <div className={`flex-1 pb-10 min-w-0 ${isLast ? 'pb-0' : ''}`}>
                {step.title && (
                ...
                {step.body && (
                  <p className="font-sans text-[18.5px] leading-[1.8] text-ink/80 m-0 mb-4 whitespace-pre-wrap">
                    {step.body}
                  </p>
                )}
                {step.image && (
        ...
        return (
          <div key={i} className="flex gap-[20px] md:gap-[26px] mb-12 last:mb-0">
            <div className="w-[44px] h-[44px] md:w-[54px] md:h-[54px] rounded-full bg-accent text-surface flex items-center justify-center font-serif text-[20px] md:text-[24px] shrink-0 mt-1">
              {num}
            </div>
