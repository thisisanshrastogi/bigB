export default function KeyTakeawaysBlock({ data, editing }) {
  ...
  const containerClasses = variant === 'card' 
    ? "bg-white border border-[#E2DFD5] rounded-[24px] p-8 my-8 shadow-sm"
    : "border-y border-[#2C4035] py-8 my-8";

  return (
    <div className={containerClasses}>
      {kicker && (
        <h4 className="font-sans text-[11px] font-[800] tracking-[0.12em] uppercase text-[#2C4035] mb-6">
          {kicker}
        </h4>
      )}
        ...
        {items.map((item, index) => {
          ...
          return (
            <li key={index} className="flex gap-4 items-start font-sans text-[16.5px] leading-[1.6] text-ink">
              {numbered ? (
                <div className="font-sans font-[800] text-[#2C4035] text-[15px] pt-[2px] shrink-0">
                  {String(index + 1).padStart(2, '0')}
                </div>
              ) : (
                <div className="w-[6px] h-[6px] rounded-full bg-[#2C4035] mt-[10px] shrink-0" />
              )}
