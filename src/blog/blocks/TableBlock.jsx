export default function TableBlock({ data, editing }) {
  ...
  const containerClass = isMinimal
    ...
    : `w-full ${isStackedMobile ? 'md:overflow-x-auto bg-transparent md:bg-surface md:border md:border-border md:shadow-sm rounded-[16px]' : 'overflow-x-auto border border-border rounded-[16px] shadow-sm'} my-8`;
  ...
  const thClass = isMinimal
    ? "font-sans text-[11px] font-[700] tracking-[0.1em] uppercase text-ink py-4 px-4 border-b-2 border-t border-ink bg-surface whitespace-nowrap"
    : "font-sans text-[11px] font-[700] tracking-[0.1em] uppercase text-muted py-4 px-4 border-b border-border bg-surface-sunken whitespace-nowrap";
  ...
  return (
    <div className="flex flex-col">
        ...
        <table className={tableClass}>
          ...
          <tbody className={isStackedMobile ? 'block md:table-row-group gap-4 flex-col flex md:table-row-group' : ''}>
            {rows.map((row, rowIndex) => {
              ...
              const zebraClass = (zebra && rowIndex % 2 === 1) ? "bg-surface-sunken" : "bg-surface";
              
              const trMobileClass = isStackedMobile 
                ? "block md:table-row border border-border md:border-0 md:border-b md:last:border-0 rounded-[12px] md:rounded-none overflow-hidden shadow-sm md:shadow-none"
              ...
              return (
                <tr key={rowIndex} className={`${trMobileClass} group ${zebraClass}`}>
                  {columns.map((col, colIndex) => {
                    ...
                    return (
                      <td key={colIndex} className={`py-4 px-4 text-ink/90 ${alignClass} ${stickyClass} ${tabularClass} ${tdMobileClass}`}>
                        {isStackedMobile && (
                          <span className="md:hidden font-bold text-[11px] uppercase tracking-wider text-muted mr-4 shrink-0">
                            {col.label}
                          </span>
                        )}
      ...
      {caption && (
        <div className="font-sans text-[13px] leading-[1.5] text-muted -mt-4 mb-8 text-center">
          {caption}
        </div>
      )}
    </div>
